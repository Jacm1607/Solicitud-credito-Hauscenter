                    ...
                    <div class="bg-white border @error($item->id) border-red-500 @enderror p-4 rounded-3xl h-full">
                        <div class="flex items-center flex-col ">
                            <div class="my-2 relative h-32 w-full sm:mb-0 mb-3">
                                <img class="w-full h-32 object-cover rounded-2xl" id="{{ $item->id }}" src="https://cdn.hauscenter.com.bo/no_imagen.jpg">
                            </div>
                            <div class="flex flex-col items-center">
                                <div class="my-2">
                                    <div class="w-full text-lg text-gray-800 font-bold leading-none text-center"> {{ $item->title }}</div>
                                    <p class="mt-2 leading-4 text-center">{{ $item->description }}</p>
                                ...
                                <div class="mb-2">
                                    <label for="input_{{ $item->id }}" class="text-center cursor-pointer uppercase text-bold bg-green-400 hover:bg-green-500 lg:px-8 px-20 ml-4 py-2 text-xs shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-green-300 hover:border-green-500 text-white rounded-full transition ease-in duration-300">
                                        SUBIR
                                    </label>
                                    <input type="file" name="{{ $item->id }}" id="input_{{ $item->id }}" class="hidden" onchange="load_file(event, '{{ $item->id }}')">
                                </div>
                            </div>
                        </div>
                    </div>
